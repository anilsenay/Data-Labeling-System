...
public class RandomLabelingMechanism extends LabelingMechanism
{
    ...
    public void assign(Dataset dataset, Instance instance,User user) {
        int maxLabelRandom=dataset.getMaxLabelPerInstance();
        // getting a random value between 1 and max labels per instance
        if (maxLabelRandom<1) {
            System.err.println("Max Label Per Instance is not valid (less than 1)");
            System.exit(1);
        }
        else { maxLabelRandom = (int) (1 + (Math.random() * dataset.getMaxLabelPerInstance()));}
